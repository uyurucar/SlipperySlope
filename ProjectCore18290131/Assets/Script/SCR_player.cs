...
public class SCR_player : MonoBehaviour
{
    ...
    [SerializeField] private float speed = 35;  //player speed
    ...
    void Update()
    {
        ...
        if(!start && Time.timeScale != 0)
        {
            ...
            if (intime > 3f) start = true;
        }
        else if (start && !isGameOver && Time.timeScale != 0 && !isPlayerCompleted)
        {
            transform.position += new Vector3(0, 0, 0.001f * speed);  //player going forward
    ...
    public IEnumerator PlayerFinished() //player crossed finish line
    {
        ...
        while(cubeCount > 0)
        {
            yield return new WaitForSeconds(0.1f);  //wait for all cubes detached
        }
        ...
        messageBox.text = "CONGRATS. LEVEL DONE. \n SCORE: " + finalscore;
        yield return new WaitForSeconds(5f);
