    ...
    private IEnumerator buildMap()
    {
        while(zValue < 47f)
        {
            ...
            else if(type == Type.upper1 || type == Type.upper2 || type == Type.upper3 || type == Type.upper4 || type == Type.upper5 || type == Type.upper6)
            {
                ...
                if(height == 0)
                {
                    Instantiate(upperwall0, new Vector3(Random.Range(-0.024f, 0.071f), 4.33f, zValue), Quaternion.Euler(0, 0, 0));
                }
                else if (height == 1)
                {
                    Instantiate(upperwall1, new Vector3(Random.Range(-0.024f, 0.071f), 4.42f, zValue), Quaternion.Euler(0, 0, 0));
                }
                else if (height == 2)
                {
                    Instantiate(upperwall2, new Vector3(Random.Range(-0.024f, 0.071f), 4.46f, zValue), Quaternion.Euler(0, 0, 0));
                }
            }
            else
            {
